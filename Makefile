...
define Package/nginx-ssl
  ...
  DEPENDS+= +NGINX_PCRE:libpcre \
	...
	+NGINX_HTTP_GZIP:zlib +NGINX_DAV:libxml2 libxslt
...
define Package/nginx-full
  ...
  DEPENDS+=+libpcre +nginx-ssl-util +zlib +libxml2 +libxslt
...
ifeq ($(CONFIG_NGINX_NJS_MODULE),y)
	ADDITIONAL_MODULES += --add-dynamic-module=$(PKG_BUILD_DIR)/nginx-njs/nginx
endif
...
define njs
  define Package/nginx-mod-njs
	$(call Package/nginx/default)
	DEPENDS:=+nginx-ssl
	TITLE:=Nginx NJS (JavaScript) module
  endef

  define Package/nginx-mod-njs/description
    Add support for NJS (JavaScript) module in Nginx.
  endef

  define Package/nginx-mod-njs/install
	$(INSTALL_DIR) $$(1)/usr/lib/nginx/modules
	$(INSTALL_BIN) $$(PKG_INSTALL_DIR)/usr/lib/nginx/modules/ngx_http_js_module.so $$(1)/usr/lib/nginx/modules
  endef

  NGINX_MODULES += nginx-mod-njs
endef
...
#$(eval $(call module,njs, +@NGINX_JS_MODULE,ngx_http_js, Enable NJS (JavaScript) module in Nginx.))
$(eval $(call njs))
$(eval $(call brotli))
$(eval $(call naxsi))
...
#通过指定版本号、下载地址、解压方法等步骤
define Download/nginx-njs
	VERSION:=3ac496802862347c5cf8f0b6e3825163dc7bb1c9
	SUBDIR:=nginx-njs
...
ifneq "$(or $(CONFIG_PACKAGE_nginx-mod-njs),$(QUILT))" ""
    $(call PatchDir,$(PKG_BUILD_DIR),$(PATCH_DIR)/nginx-njs,nginx-njs/)
endif
...
define Quilt/Refresh/Package
	...
	$(call Quilt/RefreshDir,$(PKG_BUILD_DIR),$(PATCH_DIR)/nginx-njs,nginx-njs/)
endef
